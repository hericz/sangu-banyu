...
{  db_setting = this;
   ...
   buttonOK->set_size_request(72,32);
   buttonOK->set_flags(Gtk::CAN_FOCUS);
   buttonOK->set_relief(Gtk::RELIEF_NORMAL);
   buttonBatal->set_size_request(72,32);
   buttonBatal->set_flags(Gtk::CAN_FOCUS);
   buttonBatal->set_relief(Gtk::RELIEF_NORMAL);
   fixed1->put(*buttonOK, 264, 8);
   fixed1->put(*buttonBatal, 176, 8);
   vbox2->pack_start(*frame1);
   vbox2->pack_start(*fixed1);
   db_setting->set_flags(Gtk::CAN_DEFAULT);
   db_setting->set_title(_("Setting Database"));
   db_setting->set_modal(false);
   db_setting->property_window_position().set_value(Gtk::WIN_POS_CENTER_ON_PARENT);
