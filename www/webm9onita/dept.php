...
//*/
	$nPrsh = count($prsh);
	if (count($dept)==1) {
		//echo "hanya 1 PLTD : ".$dept[0]["id_dep"].", isi c_id_dept : $c_id_dept<br>";
		if ($c_id_dept) {		// sudah ada cookie tersimpan
			//echo "Sudah ada cookie<br>";
			echo '<br /><br /><button id="btnSimpan" type="button" onclick="gantiSetting()">   Ganti   </button>';
		} else {
			//setcookie("timeRefresh", $tf, tlCookies);
			//setcookie("id_dept", 22, time());		//	$prsh[0]["id_pers"]
?>
			<script language="javascript" type="text/javascript">
				setCookie("timeRefresh", 5000, 365);
				setCookie("jmlRekHome", 16, 365);
				setCookie("id_dept", <?php echo $dept[0]["id_dep"] ?>, 365);
...
<?php 
	echo '<button id="btnSimpan" type="button" onclick="ubahSetting()">   Ubah   </button>';
	} 
?>
